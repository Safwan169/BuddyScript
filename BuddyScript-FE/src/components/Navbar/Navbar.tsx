...
const Navbar = () => {
	  ...
	  const handleLogout = async () => {
		try {
			...
			toast.error(error?.data?.message || 'Session ended locally');
		} finally {
			setShowProfileDropdown(false);
			dispatch(clearAuth());
					...
					<div className="collapse navbar-collapse" id="navbarSupportedContent">
						<div className="_header_form ms-auto">
							<form className="_header_form_grp">
								...
								<input className=" me-2 _inpt1" type="search" placeholder="input search text" aria-label="Search"/>
							</form>
						</div>
						<ul className="navbar-nav mb-2 mb-lg-0 _header_nav_list ms-auto _mar_r8">
							<li className="nav-item _header_nav_item">
								<Link href={'/'} className="nav-link _header_nav_link_active _header_nav_link" aria-current="page" >
								...
								<ul className="_nav_dropdown_list">
									...
									<li className="_nav_dropdown_list_item">
										<button
											...
											style={{ width: '100%', background: 'transparent', border: 'none', padding: 0, color: 'inherit', font: 'inherit', cursor: 'pointer', textAlign: 'left' }}
