...
type ComposerMode = 'desktop' | 'mobile';

type ComposerActionRowProps = {
  mode: ComposerMode;
  onPhotoClick: () => void;
  onVideoClick: () => void;
  onEventClick: () => void;
    ...
    return (
      <div className="_feed_inner_text_area_item">
        {actionConfig.map((action) => (
          <div key={action.key} className={`_feed_inner_text_area_bottom_${action.key} _feed_common`}>
            <button
              type="button"
              className="_feed_inner_text_area_bottom_photo_link"
              onClick={actionHandlerMap[action.key]}
            >
              <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">{action.icon}</span>
            </button>
          </div>
        ))}

        <div className="_feed_inner_text_area_btn">
          <button type="submit" className="_feed_inner_text_area_btn_link" disabled={submitting} aria-busy={submitting}>
            {submitting ? (
              <span className="spinner-border spinner-border-sm _mar_img" role="status" aria-hidden="true" />
            ) : (
